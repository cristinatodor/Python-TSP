...
def initPQueue(start_node, num_cities):
    pq = []
    for j in range(0, num_cities):
        if j != start_node:
            pq.append((distance_matrix[start_node][j], start_node, j))

    heapq.heapify(pq)

    return pq

def addPQueue(pq, node, num_cities, visited):
    for j in range(0, num_cities):
        if j != node and j not in visited:
            heapq.heappush(pq, (distance_matrix[node][j], node, j))

def bestFirstSearch(start_node, num_cities):
    tour_length = 0

    visited = []
    visited.append(start_node)

    pq = initPQueue(start_node, num_cities)
    ...
    while pq:
        if len(visited) == num_cities:
            ...
            tour_length += current[0]

            pq = []
            heapq.heapify(pq)

            visited.append(current[2])
            addPQueue(pq, current[2], num_cities, visited)

    tour_length += distance_matrix[visited[0]][visited[num_cities-1]]

    return visited, tour_length


#implementation of 2-opt algorithm as enhancement of Best First Search
...
def cost(route):
    ...
    length += distance_matrix[route[0]][route[num_cities - 1]]
...
def two_opt(route):
    ...
    return best, cost(route)


start_node = random.randint(0,num_cities-1)
print('Start node:', start_node)
tour1, tour_length = bestFirstSearch(start_node, num_cities)

tour, tour_length = two_opt(tour1)
print('Distance:', tour_length)
