...
def routeFitness(route):
    ...
    distance += distance_matrix[route[len(route)-1]][route[0]]
...
def initialPopulation(pop_size, num_cities):
    population = []

    for i in range(pop_size):
        population.append(createRoute(num_cities))
...
def rankRoutes(population):
    ...
    for i in range(len(population)):
        fitness_results[i] = routeFitness(population[i])

    return sorted(fitness_results.items(), key = operator.itemgetter(1), reverse = True)

def cumulativeSum(pairs, index):
    s = 0
...
def selection(pop_ranked):
    ...
    for i in range(len(pop_ranked)):
        fitness_sum += pop_ranked[i][1]

    for i in range(0, len(pop_ranked)):
        pick = 100*random.random()
        for i in range(0, len(pop_ranked)):
            percent = 100*cumulativeSum(pop_ranked, i)/fitness_sum
            if pick <= percent:
...
def nextGeneration(current_gen, mutation_rate):
...
def geneticAlgorithm(num_cities, pop_size, mutation_rate, generations):
    pop = initialPopulation(pop_size, num_cities)
    ...
    route_length = 1 / rankRoutes(pop)[0][1]
    return route_length, best_route


tour_length, tour = geneticAlgorithm(num_cities, pop_size=100, mutation_rate=0.01, generations=500)
